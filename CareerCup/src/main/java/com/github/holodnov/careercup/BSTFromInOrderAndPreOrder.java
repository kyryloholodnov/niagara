...
public class BSTFromInOrderAndPreOrder {
	...
	if ((inOrder == null) || (inOrder.length == 0)) {
	    throw new IllegalArgumentException(
		    "InOrder array cannot be null or empty");
	}
	if ((preOrder == null) || (preOrder.length == 0)) {
	    throw new IllegalArgumentException(
		    "PreOrder array cannot be null or empty");
	}
	if (inOrder.length != preOrder.length) {
	    throw new IllegalArgumentException(
		    "InOrder and PreOrder arrays have different sizes");
	}
	return constructTree(inOrder, 0, inOrder.length, preOrder, 0,
		preOrder.length);
    }
	...
	tree.right = constructTree(inOrder, root + 1, inEnd, preOrder, preStart
		+ 1 + (root - inStart), preEnd);
	return tree;
    }
}
